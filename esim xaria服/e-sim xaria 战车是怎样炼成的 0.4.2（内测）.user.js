...
function addIcon()
{
    ...
    {
        var $textbox = $("<input>", {type:"text",id:localStorageName, style:"vertical-align:5px;width:"+width+"px;height:25px;"}).appendTo($appendToElement);
        var localStorageSetting = localStorage.getItem(localStorageName);
        if (localStorageSetting === null)
        {
            //设置为默认值
    ...
    //输出方选择框
    addSelectSettingFrame(td, "可以打两边时选择输出方：", [0,1,2,-1], ["防守方","进攻方","尽量拿到top3","随机"], "ESX-FightDefaultSide", s_FightDefaultSide);
    ...
    addTextboxSettingFrame(td, 60, "ESX-FightWaitMin", s_AF_FightWaitMin);
    $("<span> 到 </span>").appendTo(td);
    addTextboxSettingFrame(td, 60, "ESX-FightWaitMax", s_AF_FightWaitMax);
    ...
    //双击完回原所在地
    addCheckBoxSettingFrame(td, 'BackToRegionCheckbox', '双击后飞回原所在地（请确保机票足够）', "ESX-BackToRegion", s_AWT_BackToRegion);

    //双击完回原战场
    addCheckBoxSettingFrame(td, 'BackToBattleCheckbox', '双击后回到原战场', "ESX-BackToBattle", s_AWT_BackToBattle);
...
function autoFight()
{
    ...
    function AFCheckAndHit(side)
    {
        ...
        else
        {
            ...
            ShowMessage(timeOutTime/1000+"秒后刷新("+getDateString(new Date(Date.now()+timeOutTime))+")");
