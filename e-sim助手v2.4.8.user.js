...
// @icon         https://suna.e-sim.org:3000/avatars/140351_small
...
function addIcon() {
    ...
    for (var i = 0; i < items.length; ++i) {
        ...
        var label = $("<label>", {class:"checkboxlabel", for:items[i][0], text:items[i][1]}).appendTo(td); // 选择框说明文本
...
function enhanceQuickButton() {
    ...
    function addMuStorage(citizenNumber) {
        ...
        $.each(items.reverse(), function(index, item) {
            ...
            var button = $("<button>", {id:"send" + item[3], style:"width:41px; margin:1px; border-color:" + borderColor, class:"button foundation-style smallhelp only-icon profileButton", title:"从军团仓库取" + number + "个" + item[4] + (item[5] ? ', 补足2天的用量' : '')}).appendTo(muStorage);
...
function markAlliance() {
    ...
    if (defenderId == allies[0]) {
        $('.flags-big.'+defenderName).after('<div class="fightFont">祖国</div>');
    }
    else if (allies.indexOf(defenderId) > 0) {
        $('.flags-big.'+defenderName).after('<div class="fightFont">盟国</div>');
    }
    else if (enemies.indexOf(defenderId) !== -1) {
        $('.flags-big.'+defenderName).after('<div class="fightFont">敌国</div>');
    }

    // 标记进攻方角色
    if (attackerId == allies[0]) {
        $('.flags-big.'+attackerName).after('<div class="fightFont">祖国</div>');
    }
    else if (allies.indexOf(attackerId) > 0) {
        $('.flags-big.'+attackerName).after('<div class="fightFont">盟国</div>');
    }
    else if (enemies.indexOf(attackerId) !== -1) {
        $('.flags-big.'+attackerName).after('<div class="fightFont">敌国</div>');
    }
...
function setupAlarm() {
    ...
    if (t<=10*60)
        ...
        setTimeout(function() {if ($('#alarmEnable10').is(":checked")) {alert(battleName + '倒计时10分钟');} $('#alarmEnable10').attr('disabled', true); }, (t-10*60)*1000);

    if (t<=5*60)
        $('#alarmEnable5').attr('disabled', true);
    else
        setTimeout(function() {if ($('#alarmEnable5').is(":checked")) {alert(battleName + '倒计时5分钟');} $('#alarmEnable5').attr('disabled', true); }, (t-5*60)*1000);

    if (t<=2*60)
        $('#alarmEnable2').attr('disabled', true);
    else
        setTimeout(function() {if ($('#alarmEnable2').is(":checked")) {alert(battleName + '倒计时2分钟');} $('#alarmEnable2').attr('disabled', true); }, (t-2*60)*1000);

    if (t<=1*60)
        $('#alarmEnable1').attr('disabled', true);
    else
        setTimeout(function() {if ($('#alarmEnable1').is(":checked")) {alert(battleName + '倒计时1分钟');} $('#alarmEnable1').attr('disabled', true); }, (t-1*60)*1000);
...
function setupSpectatorDetail() {
    ...
    var button = $('<button>', {id:"eh2ShowSpectatorDetail", text:"推测观战玩家", class:"foundation-style button", style:"padding:10px; margin:5px;"}).appendTo($('<div>').appendTo(mainDiv));
