...
var s_FightDefaultSide = 0; //两边都能打时默认输出方，0=防守方，1=进攻方，2=第一名伤害较低的方，-1=随机，可以在插件界面中设置

var s_AWT_BackToRegion = true; //双击完回原所在地，可以在插件界面中设置
var s_AWT_OriginRegionId = -1; //原所在地ID
var s_AWT_BackToBattle = true; //双击完回原战场，可以在插件界面中设置
var s_AWT_OriginBattleUrl; //原战场地址

var s_AF_FightForAlly = true; //优先攻击盟友方，可以在插件界面中设置
var s_AF_Allies = ["China","Taiwan"]; //自定义盟友，当两边都能打时优先打盟友方，可以在插件界面中设置
var s_AF_EHAllies = true; //从e-sim助手获取盟友，添加到自定义盟友中，可以在插件界面中设置
var s_AF_EHCoalition = true; //从e-sim助手获取联盟，添加到自定义盟友中，可以在插件界面中设置
...
var s_AF_AutoFlyToBattle = true; //当找不到可以打的战场时自动飞向s_AF_SearchBattleUrl的第一个起义战地区，可以在插件界面中设置

var s_AF_MaxWarningCounts = 7; //自动攻击最大警告数量，累计超过该值关闭自动攻击
var s_AF_FightWaitMin = 600; //攻击间隔下限，可以在插件界面中设置
var s_AF_FightWaitMax = 3000; //攻击间隔上限，可以在插件界面中设置
var s_AF_FightByHitOnly = false; //仅用hit输出（无连击），可以在插件界面中设置

var s_AF_NextRefreshExtendWaitMin = 20; //输出后下次刷新延时下限（秒），可以在插件界面中设置
var s_AF_NextRefreshExtendWaitMax = 240; //输出后下次刷新延时上限（秒），可以在插件界面中设置

var s_AF_HitUsingWeapon = 0; //使用武器攻击的次数，可以在插件界面中设置
...
function loadRecord()
{
    ...
    s_AWT_BackToRegion = (getValue("BackToRegion") == "true");
    s_AWT_OriginRegionId = parseInt(getValue("OriginRegionId"));
    s_AWT_BackToBattle = (getValue("BackToBattle") == "true");
    s_AWT_OriginBattleUrl = getValue("OriginBattleUrl");
    s_AF_FightForAlly = (getValue("FightForAlly") == "true");
    s_AF_Allies = getArrayValue("Allies");
    s_AF_EHAllies = (getValue("EHAllies") == "true");
    s_AF_EHCoalition = (getValue("EHCoalition") == "true");
    s_FightWeaponQuality = parseInt(getValue("FightWeaponQuality"));
    s_AF_FightWaitMin = parseInt(getValue("FightWaitMin"));
    s_AF_FightWaitMax = parseInt(getValue("FightWaitMax"));
    s_AF_NextRefreshExtendWaitMin = parseInt(getValue("NextRefreshExtendWaitMin"));
    s_AF_NextRefreshExtendWaitMax = parseInt(getValue("NextRefreshExtendWaitMax"));
    ...
    s_AF_AutoFlyToBattle = (getValue("AutoFlyToBattle") == "true");
}
...
{
    //-----------------------------------------------------------------------------------------------------------------------
    //Exsper's settings framework ver.EXS
    ...
    //输出方选择框
    addSelectSettingFrame(td, "可以打两边时选择输出方：", [0,1,2,-1], ["防守方","进攻方","尽量拿到top3","随机"], "FightDefaultSide", s_FightDefaultSide);

    addCheckBoxSettingFrame(td, 'FightForAllyCheckbox', '优先打盟友方', "FightForAlly", s_AF_FightForAlly);
    ...
    tr = $("<tr>", {style:"background-color:DarkSlateGray;"}).appendTo(table);
    td = $("<td>").appendTo(tr);
    var getAlliesFromEHButton = $('<button>', {text:"从E-sim助手获取盟友"}).appendTo(td);
    ...
    //搜寻网址
    addCheckBoxSettingFrame(td, 'AutoSearchBattleCheckbox', '战场页没有输出按钮时自动寻找可输出战场', "AutoSearchBattle", s_AF_AutoSearchBattle);
    tr = $("<tr>", {style:"background-color:DarkSlateGray;"}).appendTo(table);
    td = $("<td>").appendTo(tr);
    addCheckBoxSettingFrame(td, 'AutoFlyToBattleCheckbox', '如果找不到战场则飞向第一个起义战地区（渣网会卡住的QAQ）', "AutoFlyToBattle", s_AF_AutoFlyToBattle);

    tr = $("<tr>", {style:"background-color:DarkSlateGray;"}).appendTo(table);
    td = $("<td>").appendTo(tr);

    $("<span>", {text:"搜寻战场列表地址："}).appendTo(td);
    addTextboxSettingFrame(td, 300, "SearchBattleUrl", s_AF_SearchBattleUrl);
    tr = $("<tr>", {style:"background-color:DarkSlateGray;"}).appendTo(table);
    td = $("<td>").appendTo(tr);
...
function autoFight()
{
    ...
    function AFSendFightRequest(side, value)
    {
        ...
        var ajax = $.ajax({
            ...
            error: function(XMLHttpRequest, textStatus, errorThrown) {
                warningCount++;
                ShowMessage("攻击出错：错误码"+XMLHttpRequest.status+"("+warningCount+")");
                if (warningCount>s_AF_MaxWarningCounts)
                {
                    refreshingWhenCrashed();
                }
    ...
    function AFCheckCountryToFight()
    {
        ...
        if ((s_AF_FightForAlly == true) && ($.inArray(defenderName, s_AF_Allies) >= 0))
        {
        ...
        else
        {
            ...
            else if (s_FightDefaultSide==2)
            {
                var UserId = $("#userName").attr("href").split("=")[1];
                var $topDefender1 = $("#topDefender1");
                var topDefenderId = 0;
                var topDefenderDamage = 0;
                if (($topDefender1.length>0) && ($topDefender1.children().length>0))
                {
                    topDefenderId = $topDefender1.children().children('a[href^="profile.html?id="]').attr("href").split("=")[1];
                    topDefenderDamage = parseInt($topDefender1.children().children(".hitInfl").text().replace(/,/g, ''));
                }
                var $topAttacker1 = $("#topAttacker1");
                var topAttackerId = 0;
                var topAttackerDamage = 0;
                if (($topAttacker1.length>0) && ($topAttacker1.children().length>0))
                {
                    topAttackerId = $topAttacker1.children().children('a[href^="profile.html?id="]').attr("href").split("=")[1];
                    topAttackerDamage = parseInt($topAttacker1.children().children(".hitInfl").text().replace(/,/g, ''));
                }
                if (topDefenderId == UserId)
                {
                    ShowMessage("你在防守方输出为第一位，选择输出在 防守方");
                    return "defender";
                }
                else if (topAttackerId == UserId)
                {
                    ShowMessage("你在进攻方输出为第一位，选择输出在 进攻方");
                    return "attacker";
                }
                else
                {
                    var checkUserP = -1;
                    var $userA = $('a[href^="profile.html?id=429"]').parent().parent();
                    if($userA.length >0)
                    {
                        $.each($userA, function(i, userA) {
                            if($(userA).attr("class") == "topDefenders")
                            {
                                checkUserP = 0;
                            }
                            if($(userA).attr("class") == "topAttackers")
                            {
                                checkUserP = 1;
                            }
                        });
                    ...
                    if (checkUserP==0)
                    {
                        ShowMessage("你在防守方前三，选择输出在 防守方");
                        return "defender";
                    }
                    else if (checkUserP==1)
                    {
                        ShowMessage("你在进攻方前三，选择输出在 进攻方");
                        return "attacker";
                    }
                    else if (topDefenderDamage < topAttackerDamage)
                    {
                        ...
                        {
                            var sri = getRandomInt(0,2);
                            if (sri==0)
                            {
                                ShowMessage("双方第一位伤害相同或无人输出，随机选择输出在 防守方");
                                return "defender";
                            }
                            else
                            {
                                ShowMessage("双方第一位伤害相同或无人输出，随机选择输出在 进攻方");
                                return "attacker";
                            }
                        }
                }
