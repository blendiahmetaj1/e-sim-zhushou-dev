...
// ChangeLog
// 2.5.6.16：战斗显示网络错误信息；战斗方国旗修正；测试特殊道具页快捷菜单；测试在线玩家页增强功能；装备属性刷新和升级提示；请求超时设定；一些细节改进；
...
{
    var $textbox = $("<input>", {type:"text", style:"vertical-align:5px;width:"+width+"px;"}).appendTo($appendToElement);
    var localStorageSetting = localStorage.getItem(localStorageName);
    if (localStorageSetting === null)
    {
        //设置为默认值
...
function enhanceSpecialItem()
{
    ...
    var picTR = $("<tr>",{style:"background-color:Gray;"}).appendTo(quickBuffTable);
    var buttonBuyTR = $("<tr>",{style:"background-color:Gray;"}).appendTo(quickBuffTable);
    var buttonUseTR = $("<tr>",{style:"background-color:Gray;"}).appendTo(quickBuffTable);
    ...
    function setTable(siTable, picTR, buttonBuyTR, buttonUseTR)
    {
        ...
        $.each(specialItems, function(i, sin){
            ...
            if (pic.length === 0) // 无图无真相
                return true;
            var picTD = $("<td></td>").appendTo(picTR);
            ...
            //使用or购买
            var useTD = $("<td></td>").appendTo(buttonBuyTR);
            var buyTD = $("<td></td>").appendTo(buttonUseTR);
...
{
    //设置界面
    var dropDownSettingDiv = $('<div>', {class:"battleSettingsDiv", style:"overflow:auto; width:450px; height:300px; display:block; position:absolute; top:50%; left:50%; margin-left: -225px; margin-top: -150px; padding-bottom: 20px; text-align:center;background-color:lightgray;"}).appendTo($('body'));
...
function enhanceQuickButton() {
    ...
    function addMuStorage(citizenNumber) {
        ...
        var items = [
            ["bread", "Food", 5, "5-FOOD", "Q5食物", 50],
            ["gift", "Gift", 5, "5-GIFT", "Q5礼物", 50],
            ["tank", "Weapon", 5, "5-WEAPON", "Q5武器", 250],
            ["ticket ", "Ticket", 5, "5-TICKET", "Q5机票", 10],
            ["bread", "Food", 3, "3-FOOD", "Q3食物", 10],
            ["gift", "Gift", 3, "3-GIFT", "Q3礼物", 10],
            ["tank", "Weapon", 1, "1-WEAPON", "Q1武器", 250],
