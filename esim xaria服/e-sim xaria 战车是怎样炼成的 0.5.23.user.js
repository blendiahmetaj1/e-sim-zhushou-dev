...
//这里是全局变量，插件某些设置可以在下面全局变量里修改，变量涉及时间的如果没有特殊说明均为毫秒单位，涉及概率的如果没有特殊说明均为百分比
...
var s_FightDefaultSide = 0; //两边都能打时默认输出方，0=防守方，1=进攻方，2=尽量拿到top1，3=尽量拿到top3，4=尽量拿到top10，5=随机但尽量打在一边，6=打在胜场较少的那一边，-1=完全随机，★
...
var s_AF_SearchBattleOneSide = true; //寻找可以打一边的战场，●
var s_AF_SearchBattleTwoSide = true; //寻找可以打两边的战场，●
...
var s_AF_TournamentMode = false; //杯赛模式，★

var s_AF_SleepProbability = 20; //不清体概率，★
var s_AF_CancelDateTime = -1; //停止双击时间，▲

var s_AFC_TargetBattleID = -1; //需要压秒的战场ID，▲
var s_AFC_BattleTimeLeftTigger = 10; //战场于多少分钟内结束，触发压秒，●
var s_AFC_TiggerProbability = 50; //触发压秒概率，★
var s_AFC_RefreshInTimeLeftMax = 20; //压秒于结束前最大时间（秒），★
var s_AFC_RefreshInTimeLeftMin = 5; //压秒于结束前最小时间（秒），★
var s_AFC_FightWeaponQuality = 0; //压秒使用武器等级，★
var s_AFC_FightByHitOnly = false; //压秒不使用连击，★
var s_AFC_FightWaitMin = 400; //压秒攻击间隔下限，★
var s_AFC_FightWaitMax = 800; //压秒攻击间隔上限，★
var s_AFC_SearchBattleRetryInterval = 10*60; //寻找战场重试间隔（秒），●
...
function arrayTrim(array)
{
    ...
    {
        output.push(array[i].replace(/(^\s*)|(\s*$)/g, ""));
    }
    return output;
}
...
function setArrayValue(item, value, isString) { //储存数组
    ...
    if (isString === true)
    {
        localStorage.setItem(item,JSON.stringify(arrayTrim(value.split(","))));
    }
    else
    {
...
function loadRecord()
{
    ...
    s_AF_SleepProbability = loadStorage("SleepProbability",s_AF_SleepProbability);
    s_AF_CancelDateTime = loadStorage("CancelDateTime",s_AF_CancelDateTime);
    ...
    s_AFC_TargetBattleID = loadStorage("AFCTargetBattleID",s_AFC_TargetBattleID);
    s_AFC_FightWeaponQuality = loadStorage("AFCFightWeaponQuality",s_AFC_FightWeaponQuality);
    s_AFC_FightByHitOnly = loadStorage("AFCFightByHitOnly",s_AFC_FightByHitOnly);
    s_AFC_FightWaitMin = loadStorage("AFCFightWaitMin",s_AFC_FightWaitMin);
    s_AFC_FightWaitMax = loadStorage("AFCFightWaitMax",s_AFC_FightWaitMax);
    ...
    s_AF_SearchBattleFromButton = loadStorage("SearchBattleFromButton",s_AF_SearchBattleFromButton);
    ...
    s_AF_TournamentMode = loadStorage("TournamentMode",s_AF_TournamentMode);
}
...
function ResetRecord()
{
    ...
    removeValue("SleepProbability");
    removeValue("CancelDateTime");
    removeValue("AFCTiggerProbability");
    removeValue("AFCRefreshInTimeLeftMax");
    removeValue("AFCRefreshInTimeLeftMin");
    removeValue("AFCTargetBattleID");
    removeValue("AFCFightWeaponQuality");
    removeValue("AFCFightByHitOnly");
    removeValue("AFCFightWaitMin");
    removeValue("AFCFightWaitMax");
...
function addIcon()
{
    ...
    {
        var dwindow = $('<div>', {"class":"dwindow", style:"overflow:auto; width:300px; height:380px; display:block; position:fixed; top:50%; left:50%; margin-left: -225px; margin-top: -150px; padding-bottom: 20px; text-align:center;background-color:lightgray;"}).appendTo($('body'));
    ...
    // 若干功能选择框和按钮
    var tablediv = $("<div>", {style:"height:500px; overflow:auto;"}).appendTo(ESXContentDrop); //为了添加滚动条
    var table = $("<table>", {id:"ESXConfig", style:"width:100%;line-height:30px"}).appendTo(tablediv);
    ...
    //杯赛模式
    addCheckBoxSettingFrame(td, 'TournamentModeCheckbox', '杯赛模式', "TournamentMode", s_AF_TournamentMode);
    ...
    //输出方选择框
    addSelectSettingFrame(td, "可以打两边时选择输出方：", [-1,0,1,2,3,4,5,6], ["随机","防守方","进攻方","尽量拿到top1","尽量拿到top3","尽量拿到top10","随机但尽量打在一边","打在胜场较少的那一边"], "FightDefaultSide", s_FightDefaultSide);
        ...
        $.ping = function(option)
        {
            ...
            $.ajax({
                ...
                error: function(XMLHttpRequest, textStatus, errorThrown) {
                    if(textStatus == 'timeout') ShowMessage("Ping超时");
                    responseTime = new Date().getTime();
                    ping = Math.abs(requestTime - responseTime);
                    if(option.afterPing) option.afterPing(ping);
                }
    ...
    $("<span>每次刷新后均有</span>").appendTo(td);
    addTextboxSettingFrame(td, 60, "SleepProbability", s_AF_SleepProbability);
    $("<span>%的概率不进行自动清体</span>").appendTo(td);

    tr = $("<tr>", {style:"background-color:DarkSlateGray;"}).appendTo(table);
    td = $("<td>").appendTo(tr);

    $("<span>于</span>").appendTo(td);
    addTextboxSettingFrame(td, 60, "CancelDateTimeTextbox", 60);
    $("<span>分钟后停止自动清体功能</span>").appendTo(td);
    var setCancelDateTime = $('<button>', {text:"设置"}).appendTo(td);
    setCancelDateTime.click(function() {
        s_AF_CancelDateTime = Date.now()+parseInt($("#ESX-CancelDateTimeTextbox").val())*60*1000;
        setValue("CancelDateTime", s_AF_CancelDateTime);
        ShowMessage("设置在"+getDateString(new Date(s_AF_CancelDateTime))+"停止自动清体功能");
    });

    var removeCancelDateTime = $('<button>', {text:"清除"}).appendTo(td);
    removeCancelDateTime.click(function() {
        s_AF_CancelDateTime = -1;
        setValue("CancelDateTime", s_AF_CancelDateTime);
        ShowMessage("已清除设定的停止清体时间");
    });

    tr = $("<tr>", {style:"background-color:DarkSlateGray;"}).appendTo(table);
    td = $("<td>").appendTo(tr);

    $("<span>重要战场回合剩余时间小于"+s_AFC_BattleTimeLeftTigger+"分钟，则有</span>").appendTo(td);
    addTextboxSettingFrame(td, 20, "AFCTiggerProbability", s_AFC_TiggerProbability);
    $("<span>%的概率在回合结束前S</span>").appendTo(td);
    addTextboxSettingFrame(td, 20, "AFCRefreshInTimeLeftMax", s_AFC_RefreshInTimeLeftMax);
    $("<span>到S</span>").appendTo(td);
    addTextboxSettingFrame(td, 20, "AFCRefreshInTimeLeftMin", s_AFC_RefreshInTimeLeftMin);
    $("<span>之间（压秒）</span><br/>").appendTo(td);
    $("<span>每次攻击间隔（毫秒）：</span>").appendTo(td);
    addTextboxSettingFrame(td, 60, "AFCFightWaitMin", s_AFC_FightWaitMin);
    $("<span> 到 </span>").appendTo(td);
    addTextboxSettingFrame(td, 60, "AFCFightWaitMax", s_AFC_FightWaitMax);
    addSelectSettingFrame(td, "使用武器：", [0,1,2,3,4,5], ["拳头","Q1","Q2","Q3","Q4","Q5"], "AFCFightWeaponQuality", s_AFC_FightWeaponQuality);
    addCheckBoxSettingFrame(td, 'AFCFightByHitOnlyCheckbox', '不使用连击', "AFCFightByHitOnly", s_AFC_FightByHitOnly);
    $("<br/>").appendTo(td);
    var setAFCTargetBattleID = $('<button>', {text:"设置当前战场为重要战场"}).appendTo(td);
    setAFCTargetBattleID.click(function() {
        s_AFC_TargetBattleID = parseInt($('a[href^="battleStatistics.html?id="]').attr('href').replace('battleStatistics.html?id=', ''));
        setValue("AFCTargetBattleID", s_AFC_TargetBattleID);
        ShowMessage("设置战场 "+ s_AFC_TargetBattleID + " 为重要战场");
    });
    var removeAFCTargetBattleID = $('<button>', {text:"清除重要战场"}).appendTo(td);
    removeAFCTargetBattleID.click(function() {
        s_AFC_TargetBattleID = -1;
        setValue("AFCTargetBattleID", s_AFC_TargetBattleID);
        ShowMessage("已清除记录的重要战场");
    });
    ...
    tr = $("<tr>", {style:"background-color:DarkSlateGray;"}).appendTo(table);
    td = $("<td>").appendTo(tr);
    addCheckBoxSettingFrame(td, 'SearchBattleFromButtonCheckbox', '优先从左侧按钮获取战场列表网址', "SearchBattleFromButton", s_AF_SearchBattleFromButton);
...
function main()
{
    ...
    if ( tfl==null || tfl.length != 2) {
        ...
        {
            setValue("WaitForLoadCount", s_WaitForLoadCount);
            console.log(s_WaitForLoadCount+":官方脚本还未执行，xaria插件等待1秒");
            setTimeout(main, 1000);
            return;
        }
        else
    ...
    if (($("#minutesLimit").length >0 && $("#minutesLimit").text() == "") || ($("#limit_timer_time_text").length >0 && $("#limit_timer_time_text").text() == "")) {
        ...
        {
            setValue("WaitForLoadCount", s_WaitForLoadCount);
            console.log(s_WaitForLoadCount+":官方时间还未刷新，xaria插件等待1秒");
            setTimeout(main, 1000);
            return;
        }
        else
        ...
        function sendWorkAjax() {
            $.ajax({
                ...
                error: function(){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "work/ajaxSideBarWork",
                        data: null,
                        success: function(data) {
                            var taskButtonWork = $("#taskButtonWork",data);
                            if (taskButtonWork.length < 1) //没有按钮，已工作
                            {
                                //去训练界面
                                document.location = "/train.html";
                            }
...
function searchBattle()
{
    function checkBattle(url)
    {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                else if ($("#fightButton2", data).length>0) //两边
                {
                    if (s_AF_SearchBattleTwoSide == true) result = true;
                    else result = false;
                }
                else if ($("#fightButton1", data).length>0) //一边
                {
                    if (s_AF_SearchBattleOneSide == true) result = true;
                    else result = false;
                }
                else
                {
    ...
    function startSearchBattle2()
    {
        ...
        $("#battlesTable tr:gt(0)").each(function(index, tr) {
            battleLink.push($("a[href*='battle.html']", tr).attr("href"));
            battleAttacker.push(getCountryFormXflagsClass($("div.xflagsMedium:first", tr).attr("class")).replace(/-/g, " "));
            battleDefender.push(getCountryFormXflagsClass($("div.xflagsMedium:last", tr).attr("class")).replace(/-/g, " "));
            var timer = $("span.is-countdown", tr);
            if (timer.length !== 1)
            {
                battleTimer.push(-1);//未知
            }
            else
            {
                ...
                {
                    battleTimer.push(0);//已过期
                }
                else
                {
                    battleTimer.push(parseInt(countDownText[0])*60*60 + parseInt(countDownText[1])*60 + parseInt(countDownText[2]));
                }
            }
        });
    }
    ...
    function startSearchBattle()
    {
        ...
        else
        {
            ...
            {
                ShowMessage("未找到可以打的战场，"+(s_AFC_SearchBattleRetryInterval/60)+"分钟后重新寻找");
                setTimeout(function() {startSearchBattle();}, s_AFC_SearchBattleRetryInterval*1000);
            }
            else
            {
                ...
                {
                    ShowMessage("未找到可以飞的起义战，"+(s_AFC_SearchBattleRetryInterval/60)+"分钟后重新寻找");
                    setTimeout(function() {startSearchBattle();}, s_AFC_SearchBattleRetryInterval*1000);
                }
            }
        }
    ...
    if (s_AF_AutoSearchBattle == true)
    {
        if (arguments.length >= 1)
        {
            var waittime = arguments[0];
            ShowMessage(waittime+"秒后开始寻找可以输出的战场");
            setTimeout(function() {startSearchBattle();}, waittime*1000);
        }
        else
        {
...
function autoFight()
{
    ...
    {
        if ((s_AF_MaxHitUsingWeapon <0)||(s_AF_HitUsingWeapon < s_AF_MaxHitUsingWeapon)) //无限制或未超限
        {
            s_AF_HitUsingWeapon++;
            ...
            else
            {
                ...
                if ( (c<=0) || ( (c<=5) && (s_AF_FightByHitOnly == false) ) )
                {
                    return 0;
                }
                else
    ...
    {
        if (s_AF_TournamentMode == true) {
            var battleLeftTime = GetBattleLeftTime();
            var timeOutTime = getRandomInt((battleLeftTime+20)*1000, (battleLeftTime+120)*1000);
            ...
            var restore_minute;
            var restore_second;
            if ($("#minutesLimit").length>0)
            {
                restore_minute = parseInt($("#minutesLimit").text().split(":")[0]);
                restore_second = parseInt($("#secondsLimit").text());
            }
            else if ($("#limit_timer_time_text").length>0)
            {
                var splitStrings = $("#limit_timer_time_text").text().split("m");
                if (splitStrings.length <= 1)
                {
                    restore_minute = 0;
                    restore_second = parseInt(splitStrings[0].substring(0,splitStrings[0].length-1));
                }
                else
                {
                    restore_minute = parseInt(splitStrings[0]);
                    restore_second = parseInt(splitStrings[1].substring(0,splitStrings[1].length-1));
                }
            }
            if ((restore_minute<0) || isNaN(restore_minute)) restore_minute=0;
            if ((restore_second<0) || isNaN(restore_second)) restore_second=0;
            var timeOutTime = getRandomInt((restore_minute*60+restore_second+s_AF_NextRefreshExtendWaitMin)*1000, (restore_minute*60+restore_second+s_AF_NextRefreshExtendWaitMax)*1000);
            ShowMessage("将于 "+getDateString(new Date(Date.now()+timeOutTime))+" 刷新",1e6);
            setTimeout(function() {location.reload();}, timeOutTime);
        }
    }
    ...
    function AFCheckCountryToFight()
    {
        ...
        if ($('.xflagsBig').length > 0)
        {
            ...
            defenderName = defenderName.substring(defenderName.indexOf('-') + 1).replace(reg," ");
            attackerName = $('.xflagsBig:last').attr('class').split(' ').pop();
            attackerName = attackerName.substring(attackerName.indexOf('-') + 1).replace(reg," ");
        }
        ...
        else
        {
            ...
            else if (s_FightDefaultSide>=2) //top1/3/10/尽量打在一边的随机/打在胜场较少的那一边
            {
                ...
                else if( userDefenderRank<10 ) //在双方排名相同
                {
                    ...
                    {
                        ShowMessage("你在双方输出均为第"+(userDefenderRank+1)+"位，但在进攻方伤害较高，选择输出在 进攻方");
                        return "attacker";
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    else if (s_FightDefaultSide == 5) //尽量打在一边的随机
                    {
                        return returnRandomSide();
                    }
                    else if (s_FightDefaultSide == 6) //打在胜场较少的那一边
                    {
                        var attackerWinRounds = $('img[src$="red_ball.png"]').length;
                        var defenderWinRounds = $('img[src$="blue_ball.png"]').length;
                        if (attackerWinRounds > defenderWinRounds)
                        {
                            ShowMessage("防守方获胜回合数较少，选择输出在 防守方");
                            return "defender";
                        }
                        else if (attackerWinRounds < defenderWinRounds)
                        {
                            ShowMessage("进攻方获胜回合数较少，选择输出在 进攻方");
                            return "attacker";
                        }
                        else
                        {
                            ShowMessage("双方获胜回合数相同");
                            return returnRandomSide();
                        }
                    }
                    else //未知模式
    ...
    if (getUrlPage() == "battle")
    {
        if (s_AF_CancelDateTime >0)
        {
            if (Date.now()>s_AF_CancelDateTime)
            {
                ShowMessage("超过设定时间，关闭自动清体",1e6);
                return;
            }
            else
            {
                ShowMessage("即将在"+getDateString(new Date(s_AF_CancelDateTime))+"停止自动清体功能");
            }
        }
        if (s_AF_SleepProbability > 0)
        {
            var si = getRandomInt(0,100);
            if (s_AF_SleepProbability>si) //中奖
            {
                ShowMessage("因为概率设置，本次不会清体("+si+"<"+s_AF_SleepProbability+")");
                WaitForRefresh();
                return;
            }
        }
        if (s_AFC_TargetBattleID == parseInt($('a[href^="battleStatistics.html?id="]').attr('href').replace('battleStatistics.html?id=', ''))) //检查是否是重要战场
        {
            //判断是否压秒
            if (s_AFC_TiggerProbability>0)
            {
                ...
                if (s_AFC_TiggerProbability>ri) //中奖
                {
                    ...
                    if ((restore_minute<0) || isNaN(restore_minute)) restore_minute=0;
                    if ((restore_second<0) || isNaN(restore_second)) restore_second=0;
                    var nextRecoveryTime = battleLeftTime-(restore_minute*60+restore_second);
                    if ((battleLeftTime>0) && (nextRecoveryTime<0) && (battleLeftTime<s_AFC_BattleTimeLeftTigger*60))
                    {
                        //覆盖设置
                        s_FightWeaponQuality = s_AFC_FightWeaponQuality;
                        s_AF_FightByHitOnly = s_AFC_FightByHitOnly;
                        s_AF_FightWaitMin = s_AFC_FightWaitMin;
                        s_AF_FightWaitMax = s_AFC_FightWaitMax;
                        var lt = getRandomInt(s_AFC_RefreshInTimeLeftMin ,s_AFC_RefreshInTimeLeftMax);
                        var timeOutTime = (battleLeftTime-lt)*1000;
                        ShowMessage("该场为重要战场，将于 S"+lt+" 压秒",1e6);
                        setTimeout(function(){startAutoFight();}, timeOutTime);
                        return;
                    }
                    else
                    {
                        ShowMessage("该场为重要战场，程序将在回合最后"+s_AFC_BattleTimeLeftTigger+"分钟进行判断");
                        startAutoFight();
                        return;
                    }
                }
                ...
                {
                    ShowMessage("虽然该场为重要战场，但因为概率设置("+ri+"<"+s_AFC_TiggerProbability+")，本次不进行压秒判断");
                    startAutoFight();
                    return;
                }
            }
            ...
            {
                ShowMessage("虽然该场为重要战场，但是压秒概率为0，将忽略压秒设置");
                startAutoFight();
                return;
            }
        }
