...
var s_FightDefaultSide = 0; //两边都能打时默认输出方，0=防守方，1=进攻方，2=尽量拿到top1，3=尽量拿到top3，4=尽量拿到top10，5=随机但尽量打在一边，-1=完全随机，★
...
function addIcon()
{
    ...
    //输出方选择框
    addSelectSettingFrame(td, "可以打两边时选择输出方：", [-1,0,1,2,3,4,5], ["随机","防守方","进攻方","尽量拿到top1","尽量拿到top3","尽量拿到top10","随机但尽量打在一边"], "FightDefaultSide", s_FightDefaultSide);
...
function autoFight()
{
    ...
    function AFCheckCountryToFight()
    {
        ...
        else
        {
            ...
            else if (s_FightDefaultSide>=2) //top1/3/10/尽量打在一边的随机
            {
    ...
    if (getUrlPage() == "battle")
    {
        ...
        if (s_AFC_TargetBattleID == parseInt($('a[href^="battleStatistics.html?id="]').attr('href').replace('battleStatistics.html?id=', ''))) //检查是否是重要战场
        {
            ...
            if (s_AFC_TiggerProbability>0)
            {
                ...
                {
                    ShowMessage("虽然该场为重要战场，但因为概率设置，本次不进行压秒判断");
                    startAutoFight();
                    return;
                }
            }
